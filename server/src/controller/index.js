...
export const updateTableRecord = async (req, res) => {
  ...
  try {
    ...
    if (!isExist) return res.status(404).json({ message: 'Data is not found' });
    const data = await UserInfo.findByIdAndUpdate(id, {
      employeName: !employeName ? isExist.employeName : employeName,
      department: !department ? isExist.department : department,
      salary: !salary ? isExist.salary : salary,
    });
    return res.status(200).json(data);
  } catch (error) {
    return res.status(400).json({ messege: 'not updated' });
  }
};
...
export const deleteTableRecord = async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ messege: 'not deleted' });
  }
};
